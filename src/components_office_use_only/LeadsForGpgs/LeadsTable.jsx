...
function LeadsTable({ setActiveTab, activeLead, setActiveLead }) {
  ...
  const { mutateAsync: updateClient, isPending: isUpdatePending } = useUpdateClientDetails();
  ...
  useEffect(() => {
    ...
    const isManager = ManagerOptions.some(
      (opt) => opt.value === empName
    );
  ...
  const filteredRows = clientList?.filter((c) => {
    ...
    const matchesSearch = searchData
      ? Object.values(c).some(v =>
        String(v ?? "").toLowerCase().includes(searchData?.trim()?.toLowerCase())
      )
      : true;

    /* 🎯 OTHER FILTERS */
    const matchesFilters =
      (filters.LeadStatus.length === 0 || filters.LeadStatus.includes(c.LeadStatus)) &&
      ...
      editedRows.forEach((edited, index) => {
        ...
        Object.keys(original).forEach((key) => {
          ...
          if (oldVal !== newVal) {
            ...
            if (fieldLabels[key]) {
              changedFields.push(
                `${fieldLabels[key]} changed from ${oldVal || "N/A"
                } to ${newVal || "N/A"}`
              );
            }
          }
  ...
  const EditableCell = ({ rowIndex, field }) => {
    ...
    if (selectFields.includes(field)) {
      ...
      const isReasonDisabled =
        field === "Reason" &&
        editedRows[rowIndex]?.LeadStatus !== "Not Interested" && editedRows[rowIndex]?.LeadStatus !== "Follow Up";
          ...
          <div className="flex  flex-wrap gap-2 items-center mx-auto">
            ...
            {isAnyFilterApplied && (
              <button className="flex items-center bg-red-500 justify-between h-8 sm:justify-center gap-2 px-4 py-1 border border-orange-500 text-orange-600 rounded-xl hover:bg-red-400 shadow-sm transition-all whitespace-nowrap">
                <span onClick={handleClearAllFilters} className="font-medium flex  text-white items-center gap-2">
                  <RxCross2 /> Filter
                </span>
              </button>
            )}
          ...
          {paginatedRows.map((client) => (
            ...
            >
              ...
              <div className="space-y-1 text-gray-700">
                ...
                <p><b>Calling No:</b> {client.CallingNo}</p>
                <p><b>WhatsApp No:</b> {client.WhatsAppNo}</p>
                <p><b>Followup Date:</b> {client.FollowupDate || "N/A"}</p>
                <p><b>Status:</b> {client.LeadStatus}</p>
                <p><b>Reason:</b> {client.Reason || "—"}</p>
                <p><b>Field Member:</b> {client.FieldMember || "N/A"}</p>
              </div>
