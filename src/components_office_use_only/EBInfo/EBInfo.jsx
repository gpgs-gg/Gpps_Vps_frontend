...
const EbCalculation = () => {
  ...
  const { data: ebStatuses } = useDynamicValuesEBStatuses();
    ...
    const {mutateAsync: updateAssigneeMutation, isPending: isUpdateAssigneePending} = useUpdateEbAssignee();
  ...
  /* ================= RENDER ================= */
//  const paginatedRows = paginatedData.slice(
...
//   console.log("paginated rows: ", paginatedRows);
  ...
  const handleTransferSave = async () => {
    ...
    try {
      ...
      await updateAssigneeMutation({ sheetName: SHEET_NAME , formData: payload });
...
const CellWithTooltip = ({ value, fullValue, colIndex, columnKey }) => {
    ...
    const isLeftTooltip = columnKey === "UpdatedBy";
    return (
      <div
...
//     const normalizeWorkLogLines = (text) => {
//   const rawLines = normalizeAmPm(text)
//     .split("\n")
//     .map(l => l.trim())
//     .filter(Boolean);
...
//   /* ================= WORKLOGS FORMAT (useMemo) ================= */
// /* ================= WORKLOGS FORMAT (useMemo) ================= */
// /* ================= WORKLOGS FORMAT (useMemo) ================= */
// /* ================= WORKLOGS FORMAT (useMemo) ================= */
// const formattedWorkLogs = useMemo(() => {
//   if (columnKey !== "WorkLogs" || !fullValue) return null;

//   const lines = normalizeWorkLogLines(fullValue);

//   return lines.map((line, index) => {
//     if (!line.includes("]")) {
//       return (
//         <div key={index} className="text-sm font-semibold text-gray-900 mb-1">
//           {line}
//         </div>
//       );
//     }
...
//   /* ================= TOOLTIP POSITION ================= */
//   const isLeftTooltip = columnKey === "UpdatedBy";
//   const isWorkLogs = columnKey === "WorkLogs";
...
//           className={`
//             absolute top-0 z-[130] w-[350px]
//             bg-white border border-gray-300 rounded-lg shadow-xl p-3 text-xs
//             ${isLeftTooltip ? "right-full mr-2" : "left-full ml-2"}
//           `}
//         >
//           {isWorkLogs ? (
        ...
        (isLoading ? (
          ...
          <div className="mx-6 my-6">
            <div className="bg-white flex justify-between sm:block rounded-lg shadow p-4 mb-6">
              ...
              >
                Changed Assignee ({selectedLeadNos.length}) <i className="fa-solid fa-arrow-right"></i>
              </button>
            )}
              </div>
                ...
                <div className="max-h-[44vh] overflow-auto">
                  {filteredData.length === 0 ? (
                    ...
                    <table className="min-w-full border-collapse text-sm table-fixed">
                      <thead className="bg-orange-300 sticky top-0 z-[50] backdrop-blur-sm">
             
                        <tr>
                                      <th className=" sticky left-0 z-20 bg-orange-300 ">
                  <input
                    type="checkbox"
                    className="w-10 scale-150 accent-orange-500"
                    checked={
                      selectedLeadNos.length === paginatedData.length &&
                      paginatedData.length > 0
                    }
                    onChange={(e) => {
                      if (e.target.checked) {
                        setSelectedLeadNos(paginatedData.map(r => r.PropertyCode));
                      } else {
                        setSelectedLeadNos([]);
                      }
                    }}
                  />
                          ...
                          <th
                            className="px-4 py-3 font-bold text-lg text-black sticky right-0 z-[120] bg-orange-300 shadow-[-4px_0_6px_-2px_rgba(0,0,0,0.1)]"
                      ...
                      <input
                        type="checkbox"
                        className="w-10 scale-150 accent-orange-500"
                        checked={selectedLeadNos.includes(row.PropertyCode)}
                        onChange={() => handleCheckboxChange(row.PropertyCode)}
                      />
                    </td>
                            {TABLE_COLUMNS.map((key, colIndex) => (
                              <td
                              ...
                              ${colIndex < 2 ? "sticky bg-orange-300  z-20" : ""}
